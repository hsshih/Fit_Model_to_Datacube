...
def gauss_na1d(x,p):
    #### p = [const, peak_na1, peak_ratio, z_na, sigma_na]
    #### Fit the Na I D absorption line, subtract 2 gaussians from the continuum
   
    na1_rest = 5891.58
    na2_rest = 5897.55
    
    na1d = p[0] - gauss(x,[p[1],(1.+p[3])*na1_rest,p[4]]) - gauss(x,[p[1]*p[2],(1.+p[3])*na2_rest,p[4]])
    return na1d
...
def gauss_n2ha_2(x,p):
    ...
    # Allow the H-alpha sigma to be different from [N II] sigma
    halpha_rest = 6562.8
    n2_1_rest = 6548.0
    n2_2_rest = 6583.3
    n2_21_ratio = 2.94442   # n2_6583 is ~2.9444 times stronger than n2_6548
...
def myfunctgauss_na1d(p, fjac=None, x=None, y=None, err=None):
    # Fits the Na I D doublet absorption profiles
    #### p = [const, peak_na1, peak_ratio, z_na, sigma_na]
    
    model = gauss_na1d(x,p)
    vals = (y-model)/err
...
def mpfit_all(casenum = 0):
    ...
    ### p_na1d = [const, peak_na1, peak_ratio, z_na, sigma_na]
    ...
    if casenum == 0:
        ...
        #p_n2ha_2 = [0.002,0.00218,0.0203,4.,0.0127,0.0203,4.5]
        p_s2 = [0.001,0.0015,0.0015,0.0203,1.5]
        p_na1d = [0.01,0.003,0.8,0.0204,1.5]
        ...
        df1_na1d = f1.fit_na1d(p_na1d,startpix=1640, npix=80, limidx=[2,4], limiteds=[[1,1],[1,1]], limits=[[0.1,1.0],[0.,2.5]])
        ...
        df1 = pandas.read_pickle(df_file)
         df2_join = pandas.concat([df1, df1_na1d], axis=1)
        df2_join.to_pickle(df_file)
    ...
    if casenum == 1:
        ...
        filename = 'J151505_scaled_sub.fits'
        objname = 'SDSSJ151505'
        
        f1 = fit_object(filename,objname)
        
        p_o3 = [0.00002,0.001,-0.0001,2.]
        p_hb = [0.002,0.0005,0.01750,2.]
        #p_n2ha_2 = [0.003,0.00218,0.01807,3.,0.001,0.01807,3.]
        p_n2ha = [0.003,0.00218,0.01807,2.,0.001,0.01807]
        p_s2 = [0.002,0.0015,0.0015,0.01807,1.5]
        p_na1d = [0.01,0.003,0.8,0.01807,1.5]
        
        df1_o3 = f1.fit_o3(p_o3,startpix=480, npix=65)
        ...
        df1_na1d = f1.fit_na1d(p_na1d,startpix=1630, npix=80, limidx=[2,4], limiteds=[[1,1],[1,1]], limits=[[0.1,1.0],[0.,2.5]])
    ...
    if casenum == 2:
        ...
        #p_n2ha_2 = [0.01,0.02,0.029,3.4,0.001,0.029,3.4]
        p_n2ha = [0.01,0.01,0.029,2.4,0.02,0.029]
        p_s2 = [0.01,0.01,0.009,0.029,3.4]
        p_na1d = [0.01,0.003,0.8,0.029,1.5]
        ...
        df1_na1d = f1.fit_na1d(p_na1d,startpix=1680, npix=110, limidx=[2,4], limiteds=[[1,1],[1,1]], limits=[[0.1,1.0],[0.,2.5]])
        
        df_file = objname + '_fitresult.pkl'
        df_csv = objname + '_fitresult.csv'
        df1_join = pandas.concat([df1_o3, df1_hb, df1_n2ha, df1_s2,, df1_na1d], axis=1)
        df1_join.to_pickle(df_file)
        df1_join.to_csv(df_csv)
...
def mkfis_obj(df_file,objname,fitsdim):
    ...
    fitz_na = np.array(df['z_na'])

    
    fitarrays = [fit_fha, fitz_ha, fitsig_ha, fit_n2ha_ratio, fit_o3hb_ratio, fit_s2_ratio, fit_hahb_ratio, fitz_na]
    fit_filenames = ['fit_fha.fits', 'fitz_ha.fits','fitsig_ha.fits','_n2ha_ratio.fits','_o3hb_ratio.fits','_s2_6716_6730_ratio.fits','_hahb_ratio.fits', 'fitz_na1d.fits',]
...
class fit_object:
    ...
    def fit_na1d(self,p_na1d,startpix=1680, npix=110, limidx=None, limiteds=None, limits=None):
        df = mpfitgauss(self.filename, p_na1d, self.fitsdim, function=myfunctgauss_na1d, startpix=startpix, npix=npix,\
                        cols = ['const_na','peak_na1','peak_ratio','z_na','sig_na', \
                                'constna_err','pna1_err','pratio_err','zna_err','signa_err'],
                        limidx=limidx, limiteds=limiteds, limits=limits, pdfname=self.objname+'_Na1d_plot.pdf', line='na')
...
def mpfitgauss(filename, p0, fitsdim, function=myfunctgauss1,startpix=700, npix=100,
    ...
    for i in range(fitsdim[1]):
        for j in range(fitsdim[2]):
            ...
            if line is 'na':
                fitmodel = gauss_na1d(wav,m.params)
