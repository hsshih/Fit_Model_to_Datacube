...
def mkfis_obj(df_file,objname,fitsdim):
    ...
    #fit_fha = np.array(gauss_area(df['peak_ha'],df['sig_ha']))
    #fitz_ha = np.array(df['z_ha'])
    #fitsig_ha = np.array(df['sig_ha'])
    #fit_n2ha_ratio = np.array(gauss_area(df['peak_n2_1'],df['sig_n2']) * 3.94442 / gauss_area(df['peak_ha'],df['sig_ha']))
    ...
    #fit_hahb_ratio = np.array(gauss_area(df['peak_ha'],df['sig_ha']) / gauss_area(df['peak_hb'],df['sig_hb']))
...
def mkbpt(df_file,objname):
    ...
    #fit_n2ha_ratio = np.array(gauss_area(df['peak_n2_1'],df['sig_n2']) * 2.94442 / gauss_area(df['peak_ha'],df['sig_ha']))
    #fit_o3hb_ratio = np.array(gauss_area(df['peak_o3'],df['sig_o3']) / gauss_area(df['peak_hb'],df['sig_hb']))
    #fit_s2ha_ratio = np.array((gauss_area(df['peak_s2_1'],df['sig_s2']) + gauss_area(df['peak_s2_2'],df['sig_s2'])) / gauss_area(df['peak_ha'],df['sig_ha']))
...
class fit_object:
    ...
    def fit_n2ha(self,p_n2ha,startpix=2360, npix=136, limidx=None, limiteds=None, limits=None):
        ...
        #                cols = ['const_n2ha', 'peak_ha', 'z_ha', 'sig_ha', 'peak_n2_1', 'z_n2', 'sig_n2', \
        #                        'constn2ha_err', 'pha_err','zha_err','sigha_err','pn2_err','zn2_err','sign2_err'],
...
#    parinfo = gparinfo.parinfo
    ...
    #fig, ax = plt.subplots(fitsdim[1],fitsdim[2], sharex=True, sharey=True)
    ...
    for i in range(fitsdim[1]):
        for j in range(fitsdim[2]):
            ...
            #If there's an error and mpfit didn't perform fit, give it a little nudge and see if it works
            #if m.params[3] == p0[3]:
            #    print("Trying again...")
            #
            #    spec = spec * 100.
...
#                fitmodel = gauss_o3(wav,m.params)
#                plt.clf()
#                plt.plot(wav,spec,'b', wav,fitmodel,'r')
#                plt.show()
